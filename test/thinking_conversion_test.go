...
func TestThinkingE2EMatrix_Body(t *testing.T) {
	...
	cases := []thinkingTestCase{
		...
		// Case 18-extra: extra_body.google.thinking_config honored when reasoning_effort is absent
		{
			name:            "18-extra",
			from:            "openai",
			to:              "gemini",
			model:           "gemini-budget-model",
			inputJSON:       `{"model":"gemini-budget-model","messages":[{"role":"user","content":"hi"}],"extra_body":{"google":{"thinking_config":{"thinkingBudget":8192,"includeThoughts":true}}}}`,
			expectField:     "generationConfig.thinkingConfig.thinkingBudget",
			expectValue:     "8192",
			includeThoughts: "true",
			expectErr:       false,
