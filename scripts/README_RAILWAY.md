...
- `CURSOR_COMPOSER_UNRESOLVED_RECEIPT_MAX_BYTES` (default `8589934592`, or 8 GiB) - shared durable reservation ceiling for acceptance-unknown fresh-turn envelopes across overlapping bridge processes. Fresh turns initially reserve up to twice `MAX_AGENT_TURN_BYTES`, so the default supports up to 64 worst-case concurrent admissions before historical unresolved receipts; the independent free-space floor still prevents disk overcommit.
