...
- `CURSOR_COMPOSER_UNRESOLVED_RECEIPT_MAX_BYTES` (default `8589934592`, or 8 GiB) - shared durable reservation ceiling for acceptance-unknown fresh-turn envelopes across overlapping bridge processes. Reservations are EXACT and adaptive (a small initial reservation grows to the actual prepared envelope, then materializes), and the ledger is SHARDED into 16 CAS files by receipt key, so concurrent turns never contend on one global file. The ceiling counts logical reservation bytes; only UNMATERIALIZED reservations also count against the free-space floor (on-disk receipt bytes are counted once, via statfs).
...
- `CURSOR_COMPOSER_AGENT_GC` (default `0`) - maintenance-only SDK-agent collection. Durable-root scans run in an isolated worker so synchronous receipt traversal cannot block HTTP/SSE traffic. Keep it disabled until canaried, then set it to `1` during a quiescent maintenance window. Referenced agents are never mutated; stale unreferenced agents are archived, quarantined, and rechecked before deletion. Old same-scope quarantine markers also reclaim the SDK's deterministic local checkpoint directory after Cursor confirms the remote agent is archived or absent.
- `CURSOR_COMPOSER_AGENT_GC_MIN_IDLE_MS` (default `86400000`, or 24 hours) - minimum SDK agent idle age before quarantine. The separate 24-hour quarantine makes physical deletion require at least roughly 48 hours without a durable root or resume.
