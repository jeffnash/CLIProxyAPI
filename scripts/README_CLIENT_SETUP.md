...
| `CURSOR_COMPOSER_UNRESOLVED_RECEIPT_MAX_BYTES` | `8589934592` (8 GiB) | Shared-volume ceiling for durable acceptance-unknown fresh-turn envelopes. Reservations are exact/adaptive (small initial, grown to the actual envelope); a short cross-process aggregate lock makes the ceiling atomic while records remain sharded 16-way by receipt key. Only unmaterialized reservations count against the free-space floor. |
...
| `CURSOR_COMPOSER_AGENT_GC_MIN_IDLE_MS` | `86400000` | Minimum idle age before an unreferenced SDK agent is quarantined; the additional 24-hour quarantine keeps physical deletion roughly 48 hours behind the last durable use. |
