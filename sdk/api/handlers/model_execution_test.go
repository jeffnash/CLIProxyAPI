...
func TestProvidersForExecutionForcedGeminiRejectsRouterProvider(t *testing.T) {
	...
	_, _, _, errMsg := handler.providersForExecution("agents/test-agent", "agents/test-agent", false, decision, modelExecutionOptions{ForcedProvider: "gemini"})
	if errMsg == nil {
...
func TestProvidersForExecutionForcedGeminiUsesGeminiProvider(t *testing.T) {
	handler := NewBaseAPIHandlers(&sdkconfig.SDKConfig{}, nil)
	providers, model, _, errMsg := handler.providersForExecution("agents/test-agent", "agents/test-agent", false, modelRouteDecision{}, modelExecutionOptions{ForcedProvider: "gemini"})
	if errMsg != nil {
