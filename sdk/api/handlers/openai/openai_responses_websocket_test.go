...
type websocketPinnedFailoverExecutor struct {
	...
	// providerOverride replaces the default "xai" provider. Set it to a provider
	// without native websocket passthrough to exercise the HTTP-forwarded
	// pinned-failover path instead of the native replay path.
...
func (e *websocketPinnedFailoverExecutor) Identifier() string {
	if e != nil && e.providerOverride != "" {
...
func TestShouldExposeResponsesUpstreamError(t *testing.T) {
	tests := []struct {
		...
		// Quota failures stay observable (see shouldExposeResponsesUpstreamError):
		// the client must see the failed turn for pinned-auth failover to continue.
...
func TestResponsesUpstreamErrorBodyDrivesExposure(t *testing.T) {
	tests := []struct {
		...
		// Credential and transport failures stay silent: the client just
		// reconnects, and a fresh socket already implies a full context resend.
		// Quota failures are exposed so the client sees the failed turn and
		// pinned-auth failover can continue the conversation.
		...
		{name: "too many requests", status: http.StatusTooManyRequests, body: "usage limit reached", want: true},
...
func TestResponsesWebsocketHidesNonClientUpstreamDisconnectErrors(t *testing.T) {
	...
	tests := []struct {
		...
		// NOTE: quota-exhaustion disconnects are intentionally NOT hidden: quota
		// failures stay observable as error events (see
		// TestResponsesWebsocketReleasesPinnedAuthAfterQuotaError), so there is
		// no "quota exhausted" case here.
		{
...
func TestResponsesWebsocketReleasesPinnedAuthAfterQuotaError(t *testing.T) {
	...
	// HTTP-forwarded provider (no native websocket passthrough) so quota failures
	// surface as error events with pinned-auth failover instead of an HTTP-replay close.
	executor := &websocketPinnedFailoverExecutor{failStatus: http.StatusTooManyRequests, providerOverride: "test-provider"}
	manager := coreauth.NewManager(nil, selector, nil)
	manager.RegisterExecutor(executor)
