...
func shouldExposeResponsesUpstreamError(errMsg *interfaces.ErrorMessage) bool {
	...
	if status == http.StatusTooManyRequests {
		// Quota failures stay observable as error events: the client must see the
		// failed turn, and pinned-auth failover continues the conversation with
		// the next credential instead of dropping the connection silently.
		return true
	}
...
func writeResponsesWebsocketTerminalError(
	...
	if shouldReleaseResponsesWebsocketPinnedAuth(errMsg) {
		// A credential failure that releases the pinned auth must not tear down
		// the connection: the client reads the error event and continues the
		// conversation, failing over to the next credential on the following turn.
		return payload, writeResponsesWebsocketPayload(writer, wsTimelineLog, payload, time.Now()) == nil, nil
	}
