	...
	for {
		...
		if h != nil && h.SecretDLP != nil {
			...
			if errDLP != nil {
				...
				errorPayload, errWrite := writeResponsesWebsocketError(writer, wsTimelineLog, &interfaces.ErrorMessage{
					StatusCode: http.StatusUnprocessableEntity,
					Error:      errDLP,
				})
				log.Infof(
		...
		if upstreamMode == responsesWebsocketUpstreamModeWS {
			...
			// HTTP-forwarded turns pin incremental-capable auths too, so a
			// continuation reuses the same credential and a credential failure
			// releases the pin for failover instead of starting fresh.
			if lastAttemptedAuthID != "" {
				if pinnedAuth, ok := sessionAuthByID(lastAttemptedAuthID); ok && responsesWebsocketAuthSupportsIncrementalInput(pinnedAuth) {
					rememberPinnedAuth(lastAttemptedAuthID, modelName)
				}
			}
		}
