...
func (h *BaseAPIHandler) providersForExecution(modelName, originalRequestedModel string, allowImageModel bool, routeDecision modelRouteDecision, execOptions modelExecutionOptions) ([]string, string, map[string]any, *interfaces.ErrorMessage) {
	...
	if forcedProvider != "" {
		...
		return []string{forcedProvider}, normalizedModel, nil, nil
	}
...
func (h *BaseAPIHandler) getRequestDetailsWithOptions(modelName string, allowImageModel bool) (providers []string, normalizedModel string, metadata map[string]any, err *interfaces.ErrorMessage) {
	...
	if forcedProvider == "" {
		if provider, unprefixed, protocol, ok := internalconfig.FindManagedProviderByProtocolPrefix(h.CurrentConfig(), rawModel); ok {
			rawModel = unprefixed
			forcedProvider = internalconfig.ManagedProviderName(provider)
			forcedManagedProviderTransport = protocol
		} else if provider, unprefixed, ok := internalconfig.FindManagedProviderByPrefix(h.CurrentConfig(), rawModel); ok {
			rawModel = unprefixed
			forcedProvider = internalconfig.ManagedProviderName(provider)
		}
	}
	...
	if forcedProvider != "" {
		...
		if len(providers) == 0 {
			...
			return nil, "", metadata, &interfaces.ErrorMessage{
				StatusCode: http.StatusBadRequest,
