...
// isSSECommentOnlyPayload reports whether payload carries no data: every
// non-blank line is an SSE comment (starts with ':').
...
func (h *BaseAPIHandler) executeStreamWithAuthManagerFormats(ctx context.Context, entryProtocol, exitProtocol, modelName string, rawJSON []byte, alt string, allowImageModel bool, execOptions modelExecutionOptions) (<-chan []byte, http.Header, <-chan *interfaces.ErrorMessage) {
	...
	providers, normalizedModel, extraMeta, errMsg := h.providersForExecution(modelName, originalRequestedModel, allowImageModel, routeDecision, execOptions)
	if errMsg != nil {
		errChan := make(chan *interfaces.ErrorMessage, 1)
		errChan <- errMsg
		close(errChan)
	...
	// Leading SSE comments (e.g. ": keepalive") carry no data but must still reach
	// the client in order. They are buffered here so an error that arrives after
	// only comments still counts as a bootstrap failure and is retried, with the
	// buffered comments prepended to the winning attempt's first payload.
	var bootstrapCommentPrefix []byte
	// bootstrapHeadersCommitted records that an attempt already produced bytes for
	// the client, so a later bootstrap retry must not replace the committed
	// upstream headers with the retry attempt's headers.
	bootstrapHeadersCommitted := false
	readInitialStreamChunks := func() {
		for {
			...
			if !ok {
				...
				applyStreamHeaderInit()
				if bootstrapPayload == nil && len(bootstrapCommentPrefix) > 0 {
					bootstrapPayload = bootstrapCommentPrefix
					bootstrapCommentPrefix = nil
				}
				return
			}
			if chunk.Err != nil {
			...
			if isSSECommentOnlyPayload(payload) {
				bootstrapCommentPrefix = append(bootstrapCommentPrefix, payload...)
				bootstrapHeadersCommitted = true
				continue
			}
			if len(bootstrapCommentPrefix) > 0 {
				payload = append(bootstrapCommentPrefix, payload...)
				bootstrapCommentPrefix = nil
			}
	...
	bootstrapEligible := func(err error) bool {
		// A selected-execution error means the request may already have crossed
		// an acceptance boundary: it must surface, never re-enter selection.
		var scoped interface {
			RetryScope() coreexecutor.RetryScope
		}
		if errors.As(err, &scoped) && scoped != nil &&
			scoped.RetryScope() == coreexecutor.RetryScopeSelectedExecution {
			return false
		}
	...
	go func() {
		...
		if bootstrapErr != nil {
			...
			if len(bootstrapCommentPrefix) > 0 {
				bootstrapCommentPrefix = h.restoreSecretDLPStreamChunk(ctx, bootstrapCommentPrefix)
				if len(bootstrapCommentPrefix) > 0 {
					if okSendData := sendData(bootstrapCommentPrefix); !okSendData {
						completionOutcome = pluginapi.RequestCompletionCanceled
						completionStatus = 0
						if ctx != nil {
							completionErr = ctx.Err()
		...
		if bootstrapPayload != nil {
			bootstrapPayload = h.restoreSecretDLPStreamChunk(ctx, bootstrapPayload)
		...
		for {
			...
			if !ok {
				...
				if tail := h.flushSecretDLPStream(ctx); len(tail) > 0 {
					if okSendTail := sendData(tail); !okSendTail {
						completionOutcome = pluginapi.RequestCompletionCanceled
						completionStatus = 0
						if ctx != nil {
							completionErr = ctx.Err()
