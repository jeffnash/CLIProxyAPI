...
func (s *Service) registerExecutorForAuth(a *coreauth.Auth, forceReplace bool) {
	...
	case "passthru-native":
		s.coreManager.RegisterExecutor(executor.NewNativeProtocolExecutor(cfg))
	case "claude":
	...
	default:
		...
		if s.isManagedProvider(providerKey) {
			s.coreManager.RegisterExecutor(executor.NewManagedProviderExecutor(providerKey, cfg))
			return
		}
		if s.pluginHost != nil &&
