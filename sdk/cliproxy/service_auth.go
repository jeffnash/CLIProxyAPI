...
func (s *Service) completeModelRegistrationForAuthWithCache(ctx context.Context, auth *coreauth.Auth, compatCache *openAICompatibilityRegistrationCache) {
	...
	// NOTE: no ReconcileRegistryModelStates here. Reconciliation resets per-model
	// runtime error state, which must survive file-watcher add/update so retries
	// and cooldowns are not lost. The refresh path performs its own explicit
	// reconciliation where a reset is intended.
