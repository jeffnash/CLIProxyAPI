...
func TestManager_MarkResult_RespectsModelSuspendDisableLenient(t *testing.T) {
	...
	updated, ok := m.GetByID("auth-lenient")
	if !ok || updated == nil {
	...
	if !state.NextRetryAfter.IsZero() {
		t.Fatalf("expected NextRetryAfter to be zero when disable_model_suspend=true (lenient), got %v", state.NextRetryAfter)
	}
}
