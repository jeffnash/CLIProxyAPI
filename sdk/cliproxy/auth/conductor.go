...
func (m *Manager) executeStreamWithModelPool(ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, routeModel, executionModel string, execModels []string, pooled bool, aliasResult OAuthModelAliasResult) (*cliproxyexecutor.StreamResult, error) {
	...
	for idx, execModel := range execModels {
		...
		if bootstrapErr != nil {
			if errorRetryScope(bootstrapErr) == cliproxyexecutor.RetryScopeSelectedExecution {
				discardStreamChunks(streamResult.Chunks)
			...
			if result.Model != "" {
				if !isRequestScopedNotFoundResultError(result.Error) {
					if isTransientStreamDisconnectResultError(result.Error) {
						...
						disableCooling := m.cooldownDisabledForAuth(auth) || suspendDisabled
						state := ensureModelState(auth, result.Model)
						state.Unavailable = true
						state.Status = StatusError
						state.UpdatedAt = now
						...
						if isModelSupportResultError(result.Error) {
							...
							// Credential failure: suspend this model on the bad auth so fallback
							// wins and we do not re-hit invalid_grant on every request.
							if disableCooling {
								state.NextRetryAfter = time.Time{}
							} else {
							...
							case 429:
								...
								if !disableCooling {
									if result.RetryAfter != nil {
										...
										// Explicit Retry-After replaces the window without escalating
										// beyond the current level unless the window already expired.
										if state.Quota.NextRecoverAt.IsZero() || !state.Quota.NextRecoverAt.After(now) {
											// first open window / expired: leave backoffLevel as-is
											// (Retry-After path does not bump the ladder)
										}
									} else {
										// Once-per-window: concurrent failures inside an open
										// cooldown must not climb the backoff ladder again.
										next, backoffLevel = quotaCooldownAfterFailure(state.Quota, now)
										if !next.IsZero() {
											next = now.Add(capQuotaCooldown(auth, next.Sub(now)))
										}
									}
								}
								state.NextRetryAfter = next
...
func isPermanentRefreshCredentialError(err error) bool {
	if err == nil {
		return false
	}
	...
	if strings.Contains(raw, "status 401") || strings.Contains(raw, "401 unauthorized") ||
		strings.Contains(raw, "status 403") || strings.Contains(raw, "403 forbidden") {
		return true
	}
	for _, fragment := range []string{
		"invalid_grant",
...
func refreshErrorFromError(err error) *Error {
	...
	statusCode := statusCodeFromError(err)
	if isPermanentRefreshCredentialError(err) {
		return &Error{
	...
	case 429:
		...
		if !disableCooling {
			if retryAfter != nil {
				...
				// Once-per-window: do not escalate while NextRecoverAt is still open.
				next, backoffLevel = quotaCooldownAfterFailure(auth.Quota, now)
				if !next.IsZero() {
					capped := capQuotaCooldown(auth, next.Sub(now))
					if capped != next.Sub(now) {
						next = now.Add(capped)
					}
				}
			}
...
func (m *Manager) shouldRefresh(a *Auth, now time.Time) bool {
	...
	if hasTerminalRefreshFailure(a) {
		return false
	}
...
func (m *Manager) refreshAuthForRequest(ctx context.Context, id, failedAccessToken string) (*Auth, error) {
	...
	if err != nil && errors.Is(err, context.Canceled) {
		...
		}).Debug("credential refresh canceled")
		return nil, err
	}
	now := time.Now()
	if err != nil {
		terminal := isPermanentRefreshCredentialError(err)
		log.WithFields(log.Fields{
			"provider":   auth.Provider,
			"auth_index": auth.Index,
			"terminal":   terminal,
		}).Debug("credential refresh failed")
		...
		if current := m.auths[id]; current != nil {
			...
			if terminal {
				...
				current.StatusMessage = "refresh credentials rejected"
			} else {
...
func (m *Manager) applyRefreshUpdate(ctx context.Context, id string, refreshed *Auth) {
	...
	if current != nil {
		...
		if current.LastError != nil && strings.HasPrefix(strings.ToLower(strings.TrimSpace(current.LastError.Code)), "refresh_") {
			current.LastError = nil
			if current.Status == StatusError && strings.HasPrefix(strings.ToLower(strings.TrimSpace(current.StatusMessage)), "refresh ") {
				current.Status = StatusActive
				current.StatusMessage = ""
			}
			updateAggregatedAvailability(current, current.UpdatedAt)
		}
...
func debugLogAuthSelection(entry *log.Entry, auth *Auth, provider string, model string) {
	...
	switch accountType {
	case "api_key":
		entry.Infof("Use API key %s for model %s%s", util.HideAPIKey(accountInfo), model, suffix)
	case "oauth":
		ident := formatOauthIdentity(auth, provider, accountInfo)
		entry.Infof("Use OAuth %s for model %s%s", ident, model, suffix)
	}
}
