...
func (s *Server) setupRoutes() {
	...
	sharedOAuthCallbackHandler := func(c *gin.Context) {
		...
		// Provider-agnostic: resolve the provider from the pending session so
		// this path can be shared by any provider with the same redirect
		// constraint. Unknown states keep the historical devin default and
		// fail closed below when no session is pending.
		provider := "devin"
		if sessionProvider, okSession := managementHandlers.OAuthSessionProvider(state); okSession {
			provider = sessionProvider
		}
		if _, errWrite := managementHandlers.WriteOAuthCallbackFileForPendingSession(s.cfg.AuthDir, provider, state, code, errStr); errWrite != nil {
	...
	// The bare /callback path exists solely because the Devin authorization
	// page requires an exact http://127.0.0.1:<port>/callback redirect target;
	// see devinCallbackURL in management/auth_files_devin_oauth.go. The handler
	// resolves the provider from the pending session, so the path stays
	// shareable if another provider adopts the same constraint.
	s.engine.GET("/callback", sharedOAuthCallbackHandler)
	s.engine.GET("/devin/callback", sharedOAuthCallbackHandler)
