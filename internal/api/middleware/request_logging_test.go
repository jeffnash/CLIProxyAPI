...
func TestAttachRequestLogSourcesUsesLoggerLogsDir(t *testing.T) {
	...
	for _, path := range []string{"/v1/responses", "/backend-api/codex/responses"} {
		...
		for _, key := range []string{
			...
			if !strings.HasPrefix(partPath, logsDir+string(os.PathSeparator)) {
				t.Fatalf("%s part path %s is not under logs dir %s", key, partPath, logsDir)
			}
		}
		cleanupFileBodySourcesFromContext(c)
	}
}
