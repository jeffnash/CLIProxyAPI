...
var reviewedInPlaceByteWrites = map[string]reviewedInPlaceByteWrite{
	...
	"internal/runtime/executor/cursor_executor.go":          {1, "encodeConnectFrame copies the payload into a freshly allocated frame; the new buffer aliases no live gjson result"},
	"internal/durablestate/encryption.go":                   {2, "encryptEnvelope assembles nonce+ciphertext into a freshly allocated blob; only copies in, nothing aliases it"},
}
