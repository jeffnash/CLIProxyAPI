...
func (e *OpenAICompatExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if auth != nil && auth.Attributes != nil {
		if v := strings.TrimSpace(auth.Attributes["upstream_model"]); v != "" {
			...
			if strings.TrimSpace(modelForUpstream) != "" && !strings.EqualFold(modelForUpstream, baseModel) {
				translated = e.overrideModel(translated, modelForUpstream)
			}
		}
	}
	...
	translated = helps.ApplyTemperatureSuffix(translated, req.Model, opts, to.String())
