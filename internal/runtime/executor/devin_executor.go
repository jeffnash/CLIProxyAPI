	...
	if inputRes.IsArray() {
		for _, step := range inputRes.Array() {
			...
			case "function_result":
				id := devinFirstNonEmpty(step.Get("call_id").String(), step.Get("id").String())
		...
		for _, m := range messagesRes.Array() {
			...
			case "assistant":
				...
				if tcRes := m.Get("tool_calls"); tcRes.IsArray() {
					for _, tcItem := range tcRes.Array() {
						tcID := devinFirstNonEmpty(tcItem.Get("id").String(), tcItem.Get("call_id").String())
			...
			case "tool":
				id := devinFirstNonEmpty(m.Get("tool_call_id").String(), m.Get("id").String(), m.Get("call_id").String())
...
func supplementImagesFromOriginal(original []byte, prompts []helps.DevinPrompt) {
	...
	for _, m := range messages.Array() {
			...
			if content.IsArray() {
				for _, part := range content.Array() {
					...
					if partType == "tool_result" {
						toolCallID := devinFirstNonEmpty(part.Get("tool_use_id").String(), part.Get("id").String())
		...
		case "tool":
			toolCallID := devinFirstNonEmpty(m.Get("tool_call_id").String(), m.Get("id").String())
			var toolImgs []helps.DevinImage
			content := m.Get("content")
			if content.IsArray() {
