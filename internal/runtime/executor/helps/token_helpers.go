...
// where tiktoken may not accurately estimate token counts.
...
// GetTokenizer returns a cached tokenizer wrapper for the given model.
func GetTokenizer(model string) (*TokenizerWrapper, error) {
	...
	wrapper, ok := enc.(*TokenizerWrapper)
	if !ok {
		wrapper = &TokenizerWrapper{Codec: enc, AdjustmentFactor: 1.0}
	}
