...
// IsGPT5Model reports whether a model is in the GPT-5 family.
func IsGPT5Model(model string) bool {
...
// ApplyTemperatureSuffix injects a temperature suffix into a request payload.
func ApplyTemperatureSuffix(body []byte, model string, opts cliproxyexecutor.Options, provider string) []byte {
	return applyTemperatureSuffix(body, model, opts, provider)
}

// ConvertDeveloperRoleToSystem converts OpenAI chat messages with role
// "developer" to "system" for upstreams that reject developer messages.
func ConvertDeveloperRoleToSystem(payload []byte) []byte {
	...
	messages.ForEach(func(key, msg gjson.Result) bool {
		...
		path := "messages." + key.String() + ".role"
		if updated, err := sjson.SetBytes(out, path, "system"); err == nil {
			out = updated
		}
		return true
	})
