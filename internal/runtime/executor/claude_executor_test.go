...
func TestClaudeExecutor_CountTokens_AppliesCacheControlGuards(t *testing.T) {
	...
	// Custom base URLs use local estimation (see shouldUseClaudeUpstreamTokenCount),
	// so the upstream cache-control guards are exercised through countTokensUpstream
	// directly: that is the path that still sends a count_tokens request upstream.
...
func TestClaudeExecutor_CountTokens_InvalidGzipErrorBodyReturnsDecodeMessage(t *testing.T) {
	testClaudeExecutorInvalidCompressedErrorBody(t, func(executor *ClaudeExecutor, auth *cliproxyauth.Auth, payload []byte) error {
		// Custom base URLs use local estimation and never see the wire error;
		// exercise the upstream count_tokens path directly.
		_, err := executor.countTokensUpstream(context.Background(), auth, cliproxyexecutor.Request{
			Model:   "claude-3-5-sonnet-20241022",
			Payload: payload,
		}, cliproxyexecutor.Options{SourceFormat: sdktranslator.FromString("claude")})
...
func TestClaudeExecutor_StableCacheBreakpointsAttr(t *testing.T) {
	payload := []byte(`{
		"model": "muse-spark-1.3-contributor",
		"instructions": "be concise",
		"tools": [{"type":"function","name":"a","description":"a"},{"type":"function","name":"b","description":"b"}],
		"input": [
			{"type":"message","role":"user","content":[{"type":"input_text","text":"hello"}]},
			{"type":"message","role":"user","content":[{"type":"input_text","text":"next"}]}
		]
	}`)
	upstreamSSE := "event: message_start\ndata: {\"type\":\"message_start\",\"message\":{\"id\":\"msg_1\",\"model\":\"m\",\"role\":\"assistant\",\"content\":[],\"usage\":{\"input_tokens\":10,\"output_tokens\":0}}}\n\nevent: message_delta\ndata: {\"type\":\"message_delta\",\"delta\":{\"stop_reason\":\"end_turn\"},\"usage\":{\"output_tokens\":2}}\n\nevent: message_stop\ndata: {\"type\":\"message_stop\"}\n\n"

	run := func(t *testing.T, stable bool) []byte {
		var seenBody []byte
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			body, _ := io.ReadAll(r.Body)
			seenBody = bytes.Clone(body)
		...
		executor := NewClaudeExecutor(&config.Config{})
		_, err := executor.Execute(context.Background(), &cliproxyauth.Auth{Attributes: attrs}, cliproxyexecutor.Request{
			Model:   "muse-spark-1.3-contributor",
			Payload: payload,
	...
	t.Run("stable", func(t *testing.T) {
		...
		if got := gjson.GetBytes(seenBody, "system.0.cache_control.type").String(); got != "ephemeral" {
			t.Fatalf("system cache_control.type = %q, want ephemeral: %s", got, seenBody)
		}
		tools := gjson.GetBytes(seenBody, "tools").Array()
		if len(tools) == 0 {
			t.Fatalf("expected translated tools: %s", seenBody)
		}
		last := tools[len(tools)-1]
		if got := last.Get("cache_control.type").String(); got != "ephemeral" {
			t.Fatalf("last tool cache_control.type = %q, want ephemeral: %s", got, seenBody)
		}
		messages := gjson.GetBytes(seenBody, "messages").Array()
		if len(messages) == 0 {
			t.Fatalf("expected translated messages: %s", seenBody)
		}
		for i, message := range messages {
			message.Get("content").ForEach(func(_, block gjson.Result) bool {
				if block.Get("cache_control").Exists() {
					t.Fatalf("stable placement stamped messages.%d content: %s", i, seenBody)
				}
				return true
			})
		}
	...
	t.Run("rolling default", func(t *testing.T) {
		seenBody := run(t, false)
		messages := gjson.GetBytes(seenBody, "messages").Array()
		if len(messages) == 0 {
			t.Fatalf("expected translated messages: %s", seenBody)
		}
		last := messages[len(messages)-1].Get("content")
		found := false
		last.ForEach(func(_, block gjson.Result) bool {
		...
		if !found {
			t.Fatalf("default placement must stamp the latest message: %s", seenBody)
		}
	})
}
