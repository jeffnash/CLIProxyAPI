...
func codexReasoningReplayInsertIndex(inputItems []gjson.Result, replayItems [][]byte) int {
	...
	// Without replayed calls the reasoning may still belong to an already
	// complete call group: the translator's orphan repair can restore the call
	// the replay cache would otherwise supply, leaving reasoning-only replay
	// items behind. Anchor ahead of that group instead of prepending before
	// the user's opening message.
	if len(replayCallIDs) == 0 {
		if index, ok := codexPairedCallInsertIndex(inputItems); ok {
			return index
		}
	}
...
// codexPairedCallInsertIndex anchors replay items ahead of an already complete
// function_call/function_call_output group. It reports the index of the call
// that pairs with a later output sharing comparable call IDs, stopping the
// backward search at message boundaries so unrelated turns never merge.
func codexPairedCallInsertIndex(inputItems []gjson.Result) (int, bool) {
	for index, inputItem := range inputItems {
		itemType := strings.TrimSpace(inputItem.Get("type").String())
		if itemType != "function_call_output" && itemType != "custom_tool_call_output" {
			continue
		}
		outputIDs := codexReplayComparableCallIDs(inputItem.Get("call_id").String())
		if len(outputIDs) == 0 {
			continue
		}
		for callIndex := index - 1; callIndex >= 0; callIndex-- {
			callItem := inputItems[callIndex]
			callType := strings.TrimSpace(callItem.Get("type").String())
			if _, isMessage := codexReplayMessageRole(callItem); isMessage {
				break
			}
			if callType != "function_call" && callType != "custom_tool_call" {
				continue
			}
			for _, candidate := range codexReplayComparableCallIDs(callItem.Get("call_id").String()) {
