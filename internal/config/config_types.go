...
type RemoteManagement struct {
	...
	// QuotaProbeAllowedHosts is the explicit operator allow-list of hosts that
	// declarative quota probes (quota_probe metadata) may target. Probe URLs
	// whose host is not on this list are rejected before any request is sent.
	QuotaProbeAllowedHosts []string `yaml:"quota-probe-allowed-hosts"`
}
...
// PayloadDropToolsRule removes specific tools (by name) from the request's tools array.
// A tool matches by exact name or by the segment after the last "__" (so a short
// name like "get_console_message" matches an MCP tool named
// "mcp__server__get_console_message"). Matching covers both Anthropic-style tools
// (tools[].name) and OpenAI-style tools (tools[].function.name).
type PayloadDropToolsRule struct {
	// Models lists model entries with name pattern and protocol constraint.
	Models []PayloadModelRule `yaml:"models" json:"models"`
	// Tools lists tool names to remove.
...
type OpenAICompatibility struct {
	...
	// SupportsDeveloperRole controls whether OpenAI-compatible requests may include the
	// OpenAI "developer" role for this compatibility entry.
