...
// wireTransportKey identifies a reusable wire-profile transport: the combination
// of wire profile bits, resolved proxy, and the source transports whose TLS
// settings and dialers were cloned.
...
// wireTransportCache memoizes transports for the HTTP/1-only and
// disabled-compression wire profiles so repeated plugin requests share pooled
// connections. The header-ordering profile is never cached: its dialers close
// over per-request state (header order, request holder).
var wireTransportCache = helps.NewTransportCache[wireTransportKey](helps.DefaultTransportCacheCapacity)

// wireProxyInputs carries proxy resolution without allocating transports.
type wireProxyInputs struct {
	proxyStr         string
	explicitDirect   bool
	socks            bool
	ctxTransport     *http.Transport
...
func resolveWireProxyInputs(ctx context.Context, cfg *config.Config, auth *coreauth.Auth) (*wireProxyInputs, error) {
	...
	if proxyStr != "" {
		...
		if errParse != nil {
			return nil, fmt.Errorf("pluginhost: parse proxy %s: %w", proxyutil.Redact(proxyStr), errParse)
		}
		switch setting.Mode {
		case proxyutil.ModeDirect:
			in.explicitDirect = true
			proxyStr = ""
	...
	if ctx != nil {
		if ctxRoundTripper, ok := ctx.Value("cliproxy.roundtripper").(http.RoundTripper); ok && ctxRoundTripper != nil {
			if t, ok := ctxRoundTripper.(*http.Transport); ok && t != nil {
				in.ctxTransport = t
			} else if proxyStr == "" && !in.explicitDirect {
				return nil, fmt.Errorf("pluginhost: wire profile is not supported with custom context RoundTripper %T", ctxRoundTripper)
			}
		}
	}
	in.defaultTransport = http.DefaultTransport

	return in, nil
}

type wireBaseTransport struct {
	transport       *http.Transport
	callerTLSDialer func(context.Context, string, string) (net.Conn, error)
}

func buildWireBaseTransport(in *wireProxyInputs, disableCompression, forceHTTP1, wrapCustomTLSDialer bool) (*wireBaseTransport, error) {
	var baseTransport *http.Transport
	var builtByProxyutil bool
	if in.proxyStr != "" {
		builtTransport, _, errBuild := proxyutil.BuildHTTPTransport(in.proxyStr)
		if errBuild != nil {
			return nil, fmt.Errorf("pluginhost: build proxy transport for %s: %w", proxyutil.Redact(in.proxyStr), errBuild)
		}
		baseTransport = builtTransport
		builtByProxyutil = true
	}
	ctxTransport := in.ctxTransport

	// Priority 3 & 4: Context RoundTripper or Default transport
	if baseTransport == nil {
		if ctxTransport != nil {
			baseTransport = ctxTransport.Clone()
		} else if !in.explicitDirect {
			if def, ok := in.defaultTransport.(*http.Transport); ok && def != nil {
				baseTransport = def.Clone()
			} else {
				return nil, fmt.Errorf("pluginhost: wire profile is not supported with custom default RoundTripper %T", in.defaultTransport)
			}
		} else {
	...
	if forceHTTP1 && wrapCustomTLSDialer {
	...
	return &wireBaseTransport{transport: baseTransport, callerTLSDialer: callerTLSDialer}, nil
}
...
func (c *hostHTTPClient) newHTTPClientForRequest(ctx context.Context, cfg *config.Config, req pluginapi.HTTPRequest, httpReq *http.Request) (*http.Client, func(), error) {
	...
	in, errInputs := resolveWireProxyInputs(ctx, cfg, c.auth)
	if errInputs != nil {
		return nil, nil, errInputs
	}

	if !hasHeaderProfile {
		// HTTP/1-only and disabled-compression profiles share pooled
		// transports: keep-alives stay enabled and no per-request cleanup
		// runs (the cache evictor reaps idle connections on rotation).
		defTransport, _ := in.defaultTransport.(*http.Transport)
		key := wireTransportKey{
			http1Only:          profile.HTTP1Only,
			disableCompression: profile.DisableAutoCompression,
			proxy:              in.proxyStr,
			explicitDirect:     in.explicitDirect,
			ctxTransport:       in.ctxTransport,
			defaultTransport:   defTransport,
		}
		built, errBuild := wireTransportCache.Get(key, func() (*http.Transport, error) {
			base, errBase := buildWireBaseTransport(in, profile.DisableAutoCompression, forceHTTP1, true)
			if errBase != nil {
				return nil, errBase
			}
	...
	built, errBuild := buildWireBaseTransport(in, profile.DisableAutoCompression, forceHTTP1, false)
	if errBuild != nil {
		return nil, nil, errBuild
	}
	baseTransport := built.transport
	callerTLSDialer := built.callerTLSDialer
	isConfiguredSOCKS := in.socks
