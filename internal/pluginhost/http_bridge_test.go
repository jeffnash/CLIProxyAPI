...
func TestHostHTTPClientWireProfile_HTTP1OnlyReusesConnections(t *testing.T) {
	t.Parallel()
	...
	if got := newConns.Load(); got != 1 {
		t.Fatalf("HTTP/1-only profile opened %d connections for 2 requests, want 1 (pooled)", got)
	}
}

func TestHostHTTPClientWireProfile_HeaderProfileClosesIdleConnectionsOnCompletion(t *testing.T) {
	t.Parallel()
