...
func TestConvertClaudeRequestToAntigravity_ThinkingConfig(t *testing.T) {
	...
	// Thinking amount and summary visibility are independent: an effort-only
	// input sets the budget but must NOT default includeThoughts on.
	...
	if thinkingConfig.Exists() {
		...
		if thinkingConfig.Get("includeThoughts").Exists() {
			t.Errorf("includeThoughts must be absent without a summary selector, got %s", thinkingConfig.Get("includeThoughts").Raw)
		}
	} else {
