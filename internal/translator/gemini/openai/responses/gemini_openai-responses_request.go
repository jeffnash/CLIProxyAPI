...
func ConvertOpenAIResponsesRequestToGemini(modelName string, inputRawJSON []byte, stream bool) []byte {
	...
	if contents.Exists() && contents.IsArray() {
		arr := contents.Array()
		if len(arr) > 0 && openAIResponsesGeminiDroppableFinalModelPrefill(arr[len(arr)-1]) {
...
func openAIResponsesGeminiDroppableFinalModelPrefill(content gjson.Result) bool {
