...
// TestConvertClaudeRequestToOpenAI_ToolResultImageRelayedToUserMessage covers CL2:
// the image part inside a tool_result content array must survive translation.
// Since the relay contract replaced the in-tool array shape, survival means the
// tool message keeps the joined text while the image is relayed as an image_url
// part on the following user message (which the composer trailing path forwards
// to the bridge, EX3).
func TestConvertClaudeRequestToOpenAI_ToolResultImageRelayedToUserMessage(t *testing.T) {
	inputJSON := `{
		"model": "claude-3-opus",
		"messages": [
			{
	...
	if len(messages) != 3 {
		t.Fatalf("Expected 3 messages (assistant, tool, relay), got %d. Messages: %s", len(messages), resultJSON.Get("messages").Raw)
	}

	// Tool message keeps the joined text payload.
	if got := messages[1].Get("content").String(); got != "see image" {
		t.Fatalf("Expected tool content %q, got %q", "see image", got)
	}

	// Relay user message carries the image as an image_url part.
	relay := messages[2]
	if got := relay.Get("role").String(); got != "user" {
		t.Fatalf("Expected relay role %q, got %q", "user", got)
	}
	relayContent := relay.Get("content")
	if !relayContent.IsArray() {
		t.Fatalf("Expected relay content array, got %s", relayContent.Raw)
	...
	foundImage := false
	for _, item := range relayContent.Array() {
		if item.Get("type").String() == "image_url" {
			foundImage = true
			if got := item.Get("image_url.url").String(); got != "data:image/png;base64,iVBORw0KGgoAAAANSUhEUg==" {
				t.Fatalf("Unexpected relay image_url: %q", got)
			}
		}
	}
	if !foundImage {
		t.Fatalf("Expected an image_url part in relay content, got %s", relayContent.Raw)
	}
}
