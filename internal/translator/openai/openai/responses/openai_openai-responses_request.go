...
func ConvertOpenAIResponsesRequestToOpenAIChatCompletions(modelName string, inputRawJSON []byte, stream bool) []byte {
	...
	// Preserve an explicit store:false (a one-shot, non-durable turn demand).
	// Only the explicit false is surfaced; store:true is the default.
	if store := root.Get("store"); store.Exists() && store.Type == gjson.False {
		out, _ = sjson.SetBytes(out, "store", false)
	}

	// An explicit (possibly empty) top-level tools array governs whether tool
	// settings are meaningful: an empty/unconvertible array is an explicit "no
	// tools" signal, so tool_choice/parallel_tool_calls are omitted with it.
	// Without any tools array the settings may travel alongside tools from
	// another channel (e.g. additional_tools items), so they are carried.
	toolsFieldPresent := root.Get("tools").Exists()

	// Surface the conversation-stable Responses identifiers onto the translated
	// body so they remain readable for the executor's response-id->sessionID
	// map. previous_response_id MUST survive even on continuation turns whose
	// input is [function_call_output, message(user)]: it classifies that shape
	// as a continuation rather than a fresh user turn.
	hasPrevResponseID := false
	...
	if input := root.Get("input"); input.Exists() && input.IsArray() {
		...
		for _, item := range inputItems {
			...
			switch itemType {
			case "message", "":
				// Handle regular message conversion. A Responses `developer`
				// message keeps its elevated role (downstream folds it into
				// the system prompt); never downgrade it to `user`.
				...
				if content := item.Get("content"); content.Exists() && content.IsArray() {
					...
					content.ForEach(func(_, contentItem gjson.Result) bool {
						...
						case "input_image":
							// An input_image is not always a scalar image_url
							// string: resolve object/top-level-url shapes, and
							// degrade unresolvable attachments (file_id, empty)
							// to a model-visible text marker instead of an
							// empty image part.
							if imageURL := responsesInputImageURL(contentItem); imageURL != "" {
							...
							markerPart := []byte(`{"type":"text","text":""}`)
							markerPart, _ = sjson.SetBytes(markerPart, "text", responsesUnsupportedImageMarker(contentItem))
							contentItems = append(contentItems, markerPart)
						}
						return true
					})
			...
			case "function_call_output":
				...
				_, awaiting := awaitingToolOutputs[callID]
				// A previous_response_id-chained turn references tool calls that live
				// server-side in the previous response: an output carrying an explicit
				// call_id is a genuine continuation tool result, not an orphan card.
				if !awaiting && !(hasPrevResponseID && callID != "") {
	...
	if parallelToolCalls := root.Get("parallel_tool_calls"); parallelToolCalls.Exists() {
		if len(chatCompletionsTools) > 0 || !toolsFieldPresent {
			out, _ = sjson.SetBytes(out, "parallel_tool_calls", parallelToolCalls.Bool())
		}
	}

	// Convert tool_choice if present. Preserve the shapes the executor understands:
	//   - STRING                    -> pass through as a string (auto/none/required).
	//   - {type:function/custom}    -> normalized canonical chat-completions object.
	//   - {type:allowed_tools, ...} -> carry as a first-class
	//     allowed_tools object for the executor to intersect, keeping
	//     tool_choice itself as "auto" (a restriction set, not a forced tool).
	if toolChoice := root.Get("tool_choice"); toolChoice.Exists() && (len(chatCompletionsTools) > 0 || !toolsFieldPresent) {
...
// responsesInputImageURL resolves the image URL from a Responses `input_image`
// content part across the shapes real clients send. It returns "" when no
// usable URL is present (e.g. a bare `file_id` reference, or an object with no
// `url`), so the caller can degrade to a visible marker instead of emitting
// an empty `image_url` part.
//
// Accepted shapes (in priority order):
//   - scalar string:  {"type":"input_image","image_url":"data:..."}.
...
// responsesUnsupportedImageMarker renders an unresolvable image attachment as
// model-visible text so the turn is never silently emptied.
...
// responsesConversationID resolves the stable conversation identifier across
// the shapes real clients send: `conversation_id`, the `conversation` object
// form {"id": ...}, or a bare `conversation` string.
...
func convertResponsesTextFormatToChatResponseFormat(textFormat gjson.Result) []byte {
	...
	case "json_schema":
		// Already-nested form some relays emit: carry the json_schema object
		// through verbatim.
		if nested := textFormat.Get("json_schema"); nested.Exists() && nested.IsObject() {
			responseFormat := []byte(`{"type":"json_schema","json_schema":{}}`)
			responseFormat, _ = sjson.SetRawBytes(responseFormat, "json_schema", []byte(nested.Raw))
			return responseFormat
		}
		responseFormat := []byte(`{"type":"json_schema","json_schema":{}}`)
...
func chatToolOutputContentPart(item gjson.Result) []byte {
	...
	case "image_url", "input_image":
		...
		return part
	case "output_image":
		// Structured output image parts travel verbatim: the executor reads
		// the Responses-native shape downstream.
		if item.IsObject() {
...
func hasChatToolOutputImagePart(content gjson.Result) bool {
	...
	for _, item := range content.Array() {
		...
		case "image_url", "input_image":
			...
			hasImage = true
		case "output_image":
			if item.Get("image_url").Type != gjson.String {
				return false
			}
			hasImage = true
		}
