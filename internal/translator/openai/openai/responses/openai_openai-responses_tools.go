...
func mergeResponsesRequestChatTools(root gjson.Result) [][]byte {
	...
	// NOTE: server-side built-in tools (e.g. web_search) are intentionally NOT
	// carried into Chat Completions bodies: unknown tool types are rejected by
	// chat upstreams, and tool settings are omitted with them. Executors that
	// need an explicit unsupported-tool signal read the declarations from the
	// original Responses request instead.
	return merged
}
...
func convertResponsesFunctionToolToOpenAIChat(tool gjson.Result, overrideName string) ([]byte, bool) {
	...
	// Preserve an explicit strict flag from either the top level or the nested
	// function shape. Never invent it: the contract is opt-in.
	if strict := tool.Get("strict"); strict.Exists() {
		chatTool, _ = sjson.SetBytes(chatTool, "function.strict", strict.Bool())
