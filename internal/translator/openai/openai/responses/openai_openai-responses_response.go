...
func ConvertOpenAIChatCompletionsResponseToOpenAIResponses(ctx context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	...
	if !st.Started {
		// Surface the upstream chunk id verbatim as response.id, but never an
		// EMPTY one: synthesize a unique resp_ id instead, so an id-less
		// upstream cannot collapse every conversation onto one session key.
